...
usage() {
  ...
  echo "    (default) charmomics_db"
  echo " -b backup directory path to save the existing annotation configuration collection json"
  echo "    (default) /home/centos/backups/charmomics-annotation-configuration-backup  "
  echo " "
  echo " -h Prints usage"
  echo " "
  echo "Backups the existing charmomics Annotation Configuration as an Extended JSON with the current date to the backup directory."
...
${docker_exec_prefix} mongoexport --host "$mongo_host" --port "$mongo_port" --db "$database" --collection="annotation_config" --out="$target_backup_filepath"
...
${docker_exec_prefix} mongoimport --host "$mongo_host" --port "$mongo_port" --db "$database" --collection="annotation_config" --drop --file "$target_configuration_filepath" --jsonArray