...
usage() {
  ...
  echo " "
  echo "Dumps a Charmomics MongoDB database archive named 'charmomics-db-<current-date-seconds>.archive'"
  echo "to the designated output path. By default, the output path is '/home/centos/backups/charmomics-db-backup/'"
...
DOCKER_CONTAINER=$1
TARGET_PATH=~/backups/charmomics-db-backup/
...
docker exec "$DOCKER_CONTAINER" sh -c 'exec mongodump -d charmomics_db --archive' > "$TARGET_PATH/charmomics-db-$date_stamp.archive"
